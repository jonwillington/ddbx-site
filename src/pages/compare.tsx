...
const COL = "border-r border-rule";
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="flex items-center gap-1.5 rounded-full border border-separator bg-surface/40 px-3 py-2 text-caption text-foreground/85 transition-colors hover:border-brand-brown/50"
        type="button"
        onClick={() => setOpen((v) => !v)}
      >
      ...
      {open && (
        <div
          className="absolute right-0 z-50 mt-2 min-w-[180px] overflow-hidden rounded-card border border-separator bg-page py-1 shadow-float dark:bg-background"
          role="listbox"
        >
          {options.map((opt) => {
            ...
            return (
              <button
                key={opt.id}
                aria-selected={cur}
                className="flex w-full items-center gap-2 px-3 py-2 text-left text-body transition-colors hover:bg-black/5 dark:hover:bg-white/5"
                role="option"
                type="button"
                onClick={() => {
                  onChange(opt.id);
    ...
    <DefaultLayout drawerRight hideMobileCta>
      ...
      <section className="w-full">
        <header className="mb-5">
          <SectionEyebrow className="pt-5">Broker guide</SectionEyebrow>
          <h1 className="mt-2 text-balance font-semibold text-foreground display-doc">
            Compare UK trading platforms
          </h1>
          <p className="mt-4 max-w-measure text-lede text-foreground/85">
        ...
        {err && (
          <p className="text-body text-foreground/60" role="alert">
          ...
          <>
            {topPicks.length > 0 && (
              <div className="page-rail-twin mb-8 lg:hidden">
                <h2 className="mb-3 eyebrow text-foreground/50">
                  Our top picks
                </h2>
            ...
            <div className={`mb-4 ${panel({ size: "compact" })}`}>
              <div className="flex items-center gap-3">
                <label className="min-w-0 flex-1">
                  <span className="sr-only">Search platforms</span>
                  <input
                    className="w-full rounded-full border border-separator bg-transparent px-4 py-2 text-base text-foreground outline-none transition-colors placeholder:text-muted/60 focus-visible:border-brand-brown/50 focus-visible:ring-2 focus-visible:ring-brand-brown/40 sm:text-body dark:focus-visible:ring-brand-tan/40"
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                {FILTER_DEFS.map(({ key, label }) => {
                  ...
                  return (
                    <button
                      ...
                      className={
                        active
                          ? "rounded-full border border-brand-brown/50 bg-brand-brown/[0.08] px-3 py-1.5 text-caption font-medium text-brand-brown transition-colors dark:text-[#d8c4af]"
                          : "rounded-full border border-separator bg-surface/40 px-3 py-1.5 text-caption text-foreground/85 transition-colors hover:border-brand-brown/50"
                      }
                      type="button"
                      onClick={() =>
                ...
                {anyFilter && (
                  <button
                    className="ml-1 text-caption text-muted underline underline-offset-2 transition-colors hover:text-foreground"
                    type="button"
                    onClick={() => {
                      setQuery("");
            ...
            <p className="mb-3 text-caption text-foreground/40">
            ...
            <div className="hidden overflow-hidden rounded-card border border-rule bg-white dark:bg-surface md:block">
              {/* Header */}
              <div className="flex items-stretch select-none border-b border-rule bg-black/[0.04] dark:bg-white/[0.05] text-caption leading-none font-medium text-foreground/50">
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        <a
                          className="block truncate text-lede font-semibold leading-tight text-foreground hover:underline"
                          href={detailHref(b)}
                        >
                          {b.name}
                        </a>
                    ...
                    <div
                      className={`w-36 shrink-0 px-3 py-3 flex items-center text-small text-foreground/80 ${COL}`}
                    >
                      {platformFeeSummary(b.fees)}
                    </div>
                    <div
                      className={`w-28 shrink-0 px-3 py-3 flex items-center justify-end text-small tabular-nums text-foreground/80 ${COL}`}
                    >
                      {fmtMoney(b.fees.trade_commission_uk_gbp)}
                    </div>
                    <div
                      className={`w-24 shrink-0 px-3 py-3 flex items-center justify-end text-small tabular-nums text-foreground/80 ${COL}`}
                    >
            ...
            {visible.length === 0 && (
              <p className="py-10 text-center text-body text-foreground/50">
...
function TopPickCard({ broker: b }: { broker: BrokerOffer }) {
  return (
    <div className="rounded-card border border-brand-brown/30 bg-surface/60 p-4 dark:border-[#d8c4af]/25">
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3.5">
          <BrokerLogo broker={b} size={52} />
          <div>
            <a
              className="text-title text-foreground hover:underline"
              href={detailHref(b)}
            >
              {b.name}
            </a>
            <p className="mt-1 text-body text-foreground/60">{b.tagline}</p>
          </div>
        </div>
      ...
      <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-caption text-foreground/60">
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <a
          className="text-body text-foreground/55 underline underline-offset-2 hover:text-foreground"
          href={detailHref(b)}
        >
          Full review
        </a>
...
function BrokerCard({ broker: b }: { broker: BrokerOffer }) {
  return (
    <div className="rounded-card border border-separator bg-surface/40 p-4">
      <div className="flex items-start justify-between gap-3">
      ...
      <p className="mt-1.5 text-caption text-foreground/55">{b.tagline}</p>
      ...
      <div className="mt-2.5 flex flex-wrap gap-x-4 gap-y-1 text-caption text-foreground/60">
  ...
  return (
    <div aria-busy="true">
      ...
      <div className="page-rail-twin mb-8 lg:hidden">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Skeleton className="w-full rounded-card" h={200} />
          <Skeleton className="w-full rounded-card" h={200} />
        </div>
      </div>

      {/* Filter bar (search row + chip row), then the "N of M platforms" line. */}
      <Skeleton className="mb-4 w-full rounded-card" h={108} />
      ...
      <div className="hidden overflow-hidden rounded-card border border-rule bg-white dark:bg-surface md:block">
        <div className="h-7 border-b border-rule bg-black/[0.04] dark:bg-white/[0.05]" />
        <div className="divide-y divide-hairline dark:divide-separator">
          {Array.from({ length: SKELETON_ROWS }, (_, i) => (
            <div key={i} className="flex items-center gap-3 px-3 py-3">
              <Skeleton className="shrink-0 rounded-control" h={44} w={44} />
      ...
      <div className="space-y-3 md:hidden">
        {Array.from({ length: 4 }, (_, i) => (
          <Skeleton key={i} className="w-full rounded-card" h={244} />
        ))}
      </div>
    </div>
  );
