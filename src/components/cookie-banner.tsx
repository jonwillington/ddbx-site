...
export function CookieBanner() {
  ...
  return (
    <div
      aria-hidden={!inView}
      className={`shell-center fixed inset-x-0 bottom-4 z-50 hidden md:flex justify-center px-4 pointer-events-none transition-[transform,opacity] ease-out ${hasRightSidebar ? "lg:right-80" : ""} ${inView ? "translate-y-0 opacity-100 duration-300" : "translate-y-24 opacity-0 duration-300"}`}
    >
      <div
        className={`pointer-events-auto flex items-center gap-3 rounded-card px-4 py-2 text-small text-foreground/70 ${glass()}`}
      >
        ...
        <button
          className={`${BUTTON_RADIUS} ${BUTTON_FILLED} px-3 py-1 text-small font-medium transition-colors`}
          onClick={handleAgree}
        >
          Agree to cookies
        </button>
