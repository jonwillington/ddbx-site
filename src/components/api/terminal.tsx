...
export function Terminal({
  ...
  return (
    ...
    >
      ...
      <div
        className={`overflow-x-auto p-4 font-mono text-[12.5px] leading-[1.6] text-[#f5f0e8]/90 ${bodyClassName}`}
      >
        {children}
      </div>
    </div>
...
/** Values a JsonBlock can render. `gain` marks a directional number so it can
 *  take the one green in the palette; `elide` stands in for a trimmed array,
 *  rendering as `[ … 4 items ]` while keeping its key so the reader can still
 *  see which field was abbreviated. */
