...
//
// ---------------------------------------------------------------------------
// oneLiner, updated, sources
// ---------------------------------------------------------------------------
//
// `oneLiner` is the definition in a single sentence, rendered in a sheet under
// the h1 before the prose starts. It is written to be liftable — the answer on
// its own, with no "as discussed above" — because that is the unit a search
// result quotes. `description` is the meta description and stays a description
// of the page; the two are not interchangeable.
//
// `updated` and `sources` exist because a definitional page with no date and
// no citation is indistinguishable from generated filler. `sources` links the
// primary instrument only — the regulation, the form — and is omitted rather
// than padded with secondary commentary where no certain primary URL exists.

/** Which host publishes an entry. See the header — exactly one per entry. */
export const OWNER_HOST = { uk: "ddbx.uk", us: "ddbx.us" };

/** Index groupings. The glossary answers two different questions — what the
 *  filings are, and how to read one — and a flat list of ten makes a reader
 *  who wants the second scan past six of the first. */
export const GROUPS = [
  { id: "filings", label: "The filings" },
...
export const ENTRIES = [
  {
    ...
    oneLiner:
      "A PDMR is anyone senior enough at a listed company that their own dealings in its shares have to be disclosed to the market.",
    updated: "2026-07-26",
    ...
    sources: [
      {
        label: "Market Abuse Regulation, Article 19 — legislation.gov.uk",
        url: "https://www.legislation.gov.uk/eur/2014/596/article/19",
      },
    ],
  },
  {
    ...
    oneLiner:
      "MAR Article 19 is the rule that turns a UK or EU director’s share dealing into a public announcement, within a deadline measured in business days.",
    updated: "2026-07-26",
    liveData: "recent",
  ...
  },
  {
    ...
    oneLiner:
      "A closed period is the stretch before a company’s results during which its directors are barred from dealing in its shares.",
    updated: "2026-07-26",
    liveData: null,
    body: [
  ...
  },
  {
    ...
    oneLiner:
      "An RNS is the regulatory feed a UK listed company publishes through, and where a director’s share purchase becomes public.",
    updated: "2026-07-26",
    liveData: "recent",
  ...
  },
  {
    ...
    oneLiner:
      "An open-market buy is an insider paying the prevailing market price with their own money — not a share award vesting, an option exercise or a placing.",
    updated: "2026-07-26",
  ...
  },
  {
    ...
    oneLiner:
      "Cluster buying is several insiders at the same company buying within a short window of each other, which is harder to explain away than any single purchase.",
    updated: "2026-07-26",
    liveData: "clusters",
  ...
  },
  {
    ...
    oneLiner:
      "A director buying tells you someone with better information chose to increase their exposure with their own money — not that the shares will go up.",
    updated: "2026-07-26",
    liveData: "recent",
    ...
    ctaTerm: "what a director buy means",
  },
  {
    slug: "form-4",
    owner: "us",
    ...
    oneLiner:
      "Form 4 is the filing a US officer, director or ten-percent owner must submit within two business days of trading their own company’s stock.",
    updated: "2026-07-26",
    liveData: "recent",
    ...
    ctaSlot: "alert",
    sources: [
      { label: "SEC Form 4 — sec.gov", url: "https://www.sec.gov/files/form4.pdf" },
    ],
  },
  {
    slug: "rule-10b5-1",
    ...
    oneLiner:
      "A 10b5-1 plan is a trading schedule set in advance, so a trade executed under one reflects a decision taken months earlier rather than a view held today.",
    updated: "2026-07-26",
    liveData: null,
    body: [
    ...
    related: ["form-4"],
    ctaTerm: "10b5-1 plans",
    // No live module on this page — the band is the only thing after the prose,
    // so it earns a line written for the reader who just learned that a filing
    // can describe a decision made months ago.
    cta: {
      headline: "Know which trades were planned and which were decided.",
      body: "The app reads every filing and says what kind of transaction it was, so a sale that executed off a schedule never reaches you looking like conviction.",
    },
  },
  {
    slug: "stock-act",
    ...
    oneLiner:
      "The STOCK Act is the 2012 law requiring members of Congress and senior staff to disclose their securities transactions.",
    updated: "2026-07-26",
    liveData: null,
    body: [
    ...
    related: ["form-4"],
    ctaTerm: "the STOCK Act",
    // Same reasoning as rule-10b5-1: nothing but prose above the band. The
    // honest ask here is the contrast — we don’t publish congressional
    // filings, and pretending otherwise to sell an install would be a lie.
    cta: {
      headline: "Congress files in weeks. Company insiders file in days.",
      body: "ddbx covers the corporate side: every director and officer purchase, pushed the day it discloses, rated, with the reasoning written out.",
    },
  },
];
 ...
 *
 *  Formatted here rather than at each call site so the SPA and the pre-render
 *  can’t print the same date differently, and by hand rather than through Intl
 *  so the output doesn’t shift with the renderer’s locale or time zone — a
 *  date parsed as UTC midnight and formatted in New York is the day before. */
